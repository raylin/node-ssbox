...
function _err(msg) {
  ...
  err = (msg && msg.error) ? msg.error : 'Unkown Error';
...
SSBoxClient.prototype.account = function (callback) {
  this._request.get({
    ...
    if (!err && res.statusCode === 200) {
      ...
      callback(err || body ? _err(body) : _err());
    }
  });
};
...
SSBoxClient.prototype.downloadFolder = function (dPath, destFile, callback) {
  var gzipper = zlib.createGzip({level: 9}),
  ...
  this.fileListRecursive(dPath, function (err, fileList) {
    if (!err && fileList) {
      archive.pipe(gzipper).pipe(output);
      
      _.forEach(fileList, function (file) {
        archive.append(this.get(file.path), { name: file.path});
      }.bind(this));
